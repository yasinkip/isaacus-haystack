"""Tests for the Isaacus Embedder component. Run `pytest --asyncio-mode=auto` to execute all tests."""
...
def test_text_embedder_runs_and_returns_vector():
    with patch("requests.post", _fake_post):
        emb = IsaacusTextEmbedder(
            api_key=Secret.from_token("x"), model="kanon-2-embedder"
        )
        out = emb.run("hello")
...
def test_document_embedder_sets_embeddings_on_documents():
    with patch("requests.post", _fake_post):
        ...
        emb = IsaacusDocumentEmbedder(
            api_key=Secret.from_token("x"), batch_size=2, model="kanon-2-embedder"
        )
        out = emb.run(docs)
        docs2 = out["documents"]
